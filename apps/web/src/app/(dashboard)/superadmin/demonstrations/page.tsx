...
const STATUT_MAP: Record<string, { label: string; couleur: 'info' | 'success' | 'danger' | 'neutral' }> = {
  PLANIFIEE: { label: 'Planifiée', couleur: 'info' },
  REALISEE: { label: 'Réalisée ✓', couleur: 'success' },
  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto">
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-black text-text-main">Démonstrations</h1>
          <p className="text-sm text-text-muted">Planification et suivi des démos commerciales</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
        {[
          ...
          { label: 'Planifiées', val: stats.planifiees, c: '#0ea5e9' },
          { label: 'Réalisées', val: stats.realisees, c: '#009E00' },
          { label: 'Annulées', val: stats.annulees, c: '#E60000' },
          { label: 'Taux réal.', val: stats.tauxRealisation + '%', c: '#FFD000' },
        ].map(k => (
          ...
          <div key={d.id} onClick={() => setSelected(d.id)}
            ...
            <div className="space-y-1.5 text-sm">
              <div className="flex items-center gap-2 text-text-muted">
                <span>📅</span><span>{d.date} à {d.heure}</span>
              </div>
              <div className="flex items-center gap-2 text-text-muted">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setSelected(null)}>
          ...
          <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-lg w-full p-6" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              {[
                ['Date', `${detail.date} ${detail.heure}`],
                ['Mode', MODE_MAP[detail.mode]],
                ['Fuseau', detail.fuseau],
                ['Animateur', detail.animateur],
                ['Statut', STATUT_MAP[detail.statut]?.label],
                ['Email', detail.email],
              ].map(([k, v]) => (
            ...
            {detail.notes && (
              <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-xl p-4 mb-4">
                <p className="text-xs font-bold text-yellow-700 dark:text-yellow-400 mb-1">Notes</p>
            ...
            <div className="flex gap-2 flex-wrap">
              {detail.statut === 'PLANIFIEE' && (
                <>
                  <button className="flex-1 px-4 py-2.5 rounded-xl bg-brand-green text-white text-sm font-bold">✓ Marquer réalisée</button>
                  <button className="px-4 py-2.5 rounded-xl border border-red-300 text-red-600 text-sm font-bold">Annuler</button>
                </>
              )}
              {detail.statut === 'REALISEE' && (
                <button className="flex-1 px-4 py-2.5 rounded-xl bg-brand-green text-white text-sm font-bold">📄 Créer une offre</button>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
          ...
          <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-lg w-full p-6" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-start mb-5">
              <h2 className="text-xl font-black text-text-main">Planifier une démo</h2>
              <button onClick={() => setShowForm(false)} className="text-text-muted hover:text-text-main text-xl">✕</button>
            </div>
            <div className="space-y-3">
              {[
                { label: 'Prospect (email)', type: 'email', placeholder: 'prospect@entreprise.com' },
              ...
              <div>
                <label className="text-xs font-bold text-text-muted block mb-1">Mode</label>
