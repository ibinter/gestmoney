...
// ─── Constantes UI ────────────────────────────────────────────────────────
const STATUT_CONFIG: Record<StatutTicket, { label: string; couleur: string; icone: React.ReactNode }> = {
  ouvert:   { label: 'Ouvert',    couleur: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300',     icone: <Clock size={11} /> },
  en_cours: { label: 'En cours',  couleur: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300', icone: <AlertTriangle size={11} /> },
  resolu:   { label: 'Résolu',    couleur: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300', icone: <CheckCircle2 size={11} /> },
  ferme:    { label: 'Fermé',     couleur: 'bg-gray-100 text-gray-500 dark:bg-white/08 dark:text-gray-400',        icone: <X size={11} /> },
};

const PRIORITE_CONFIG: Record<PrioriteTicket, { label: string; couleur: string; point: string }> = {
  basse:   { label: 'Basse',   couleur: 'text-gray-400',  point: 'bg-gray-300' },
  normale: { label: 'Normale', couleur: 'text-blue-500',  point: 'bg-blue-400' },
  haute:   { label: 'Haute',   couleur: 'text-orange-500',point: 'bg-orange-400' },
  urgente: { label: 'Urgente', couleur: 'text-red-500',   point: 'bg-red-500 animate-pulse' },
};

const CATEGORIES: { value: CategorieTicket; label: string }[] = [
  { value: 'technique',   label: 'Problème technique' },
  { value: 'transaction', label: 'Transaction' },
  { value: 'float',       label: 'Float / Solde' },
  { value: 'agent',       label: 'Agent / Accès' },
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-lg bg-surface dark:bg-[hsl(0_0%_10%)] rounded-2xl shadow-2xl border border-gray-100 dark:border-white/10 overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-white/08">
          <h2 className="font-bold text-text-main">Nouveau ticket de support</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block text-xs font-semibold text-text-muted mb-1.5">Titre du problème *</label>
            <input
              type="text"
              value={form.titre}
              onChange={(e) => set('titre', e.target.value)}
              placeholder="Ex : Transaction bloquée, agent ne peut pas se connecter…"
              required
          ...
          <div>
            <label className="block text-xs font-semibold text-text-muted mb-1.5">Description détaillée *</label>
            <textarea
              value={form.description}
              onChange={(e) => set('description', e.target.value)}
              rows={5}
              required
              placeholder="Décrivez le problème en détail : étapes pour reproduire, messages d'erreur, références de transactions concernées…"
          ...
          <div className="flex items-center justify-between pt-2">
            <button type="button" onClick={onFermer} className="text-sm text-text-muted hover:text-text-main transition-colors px-3 py-2">Annuler</button>
            <button
              type="submit"
            ...
            >
              {envoi ? 'Envoi…' : <><Send size={14} /> Envoyer le ticket</>}
            </button>
          </div>
        </form>
      </div>
        ...
        <div className="px-6 py-5 border-b border-gray-100 dark:border-white/08">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              ...
              <p className="text-xs text-text-muted mt-1">Ouvert {formatRelativeTime(ticket.dateCreation)} · Catégorie : {CATEGORIES.find(c => c.value === ticket.categorie)?.label}</p>
            </div>
          </div>
        </div>
          ...
          {messages.map((msg) => (
            <div key={msg.id} className={clsx('px-6 py-4', msg.role === 'support' && 'bg-primary/03')}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-semibold text-text-main">{msg.auteur}</span>
                {msg.role === 'support' && <span className="text-[10px] bg-primary/10 text-primary px-2 py-0.5 rounded-full font-semibold">Support IBIG Soft</span>}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'Total',   val: stats.total,   col: 'text-text-main' },
          { label: 'Ouverts', val: stats.ouverts, col: 'text-blue-500' },
          { label: 'En cours', val: stats.enCours, col: 'text-yellow-500' },
          { label: 'Résolus', val: stats.resolus, col: 'text-green-500' },
        ].map((k) => (
      ...
      <div className="flex gap-2 flex-wrap">
        {(['tous', 'ouvert', 'en_cours', 'resolu', 'ferme'] as const).map((s) => (
          ...
          >
            {s === 'tous' ? 'Tous' : STATUT_CONFIG[s as StatutTicket].label}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-2">
        {ticketsFiltres.length === 0 ? (
          <div className="bg-white dark:bg-white/03 rounded-2xl border border-gray-100 dark:border-white/08 px-6 py-12 text-center">
            ...
            <p className="text-text-muted text-sm">Aucun ticket dans cette catégorie.</p>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    <span className="text-[10px] text-text-muted bg-gray-100 dark:bg-white/08 px-2 py-0.5 rounded-full">
                      {CATEGORIES.find((c) => c.value === t.categorie)?.label}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-3 mt-1.5 text-xs text-text-muted">
                    <span className="flex items-center gap-1"><MessageSquare size={11} /> {t.nbMessages} messages</span>
                    <span>Mis à jour {formatRelativeTime(t.dateMaj)}</span>
                  </div>
                </div>
      ...
      <div className="bg-gradient-to-r from-primary/08 to-[#FFD000]/08 rounded-2xl border border-primary/15 p-5 flex items-center justify-between gap-4 flex-wrap">
        <div>
          <p className="font-semibold text-text-main text-sm">Urgence ? Contactez-nous directement</p>
          <p className="text-xs text-text-muted mt-0.5">Réponse garantie en moins de 2h pour les tickets urgents.</p>
        </div>
